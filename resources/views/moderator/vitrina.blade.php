    ...
    <section class="content">
        <div class="container-fluid">
          <div class="row align-items-center mb-3 pt-3">
            <div class="col-10" >
              <h5 class="pt-2 mb-2">Список товаров <span data-bs-toggle="tooltip" data-bs-placement="right" title="Количество товаров">({{count($vitrinas)}})</span></h5>
            </div>
            <div class="col-2" >
              <a class="btn btn-sm btn-success float-right text-nowrap" data-bs-toggle="tooltip" data-bs-placement="left" title="Добавить товар" href="{{ route('moderator_vitrina_create', ['for_action' => $for_action])}}">
            ...
            <div class="card mb-3 todo-list2 accordion{{$vitrina->id}}"  id="accordion{{$vitrina->id}}">
              <div class="row no-gutters align-items-center">
                <div class="col-md-3">
                                  <div id="carouselExampleIndicators{{$loop->iteration}}" data-interval="false" data-keyboard="false"  data-touch="true" class="carousel slide  shadow-sm" data-ride="carousel">
                                    ...
                                    @if(count($vitrinaimgs->where('vitrina_id', $vitrina->id)) > 1)
                                    <a style="width: 25px;" class="carousel-control-prev" href="#carouselExampleIndicators{{$loop->iteration}}" role="button" data-slide="prev">
                                      <span class="carousel-control-custom-icon" aria-hidden="true">
                                        <i class="fas fa-chevron-left" style="font-size: 25px; color: red;"></i>
                                      </span>
                                      <span class="sr-only">Previous</span>
                                    </a>
                                    <a style="width: 25px;" class="carousel-control-next" href="#carouselExampleIndicators{{$loop->iteration}}" role="button" data-slide="next">
                                      <span class="carousel-control-custom-icon" aria-hidden="true">
                  ...
                  <div class="row mr-0 ml-0  align-items-center"> 
                    <div class="col-md-11 pr-0 pl-0">
                      <div class="card-body pr-2 pl-2 pt-3 pb-3">
                        <div class="row">
                          <div class="col">
                            <h6 class=" text-truncate" title="{{$vitrina->title}}">{{$vitrina->title}}</h6>
                          </div>
                          <div class="col-auto">
                                <!-- Main content 
                            <a class="btn btn-tool w-1001" href="{{ route('moderator_kurs_certificate', $vitrina['id']) }}">
                        ...
                        <small class="text-muted"> Язык: @if(intval($vitrina->language) != 4 ){{$vitrina->languages['language']}}@else - @endif / Жеткирүү: 
                        ...
                        <p class="card-text text-muted mb-0 mt-1">Категория: {{$vitrina->vitrina_category->title}} / {{$vitrina->vitrina_podcategory->title}}</p>
                        <div class="row align-items-center ml-0 mr-0 mt-3">
                          ...
                          <div class="col border text-center shadow-sm two dropup" style="border-top-left-radius: 3px; border-bottom-left-radius: 3px;">
                            <a class="" data-toggle="dropdown" id="dropdownMenuOffset2" type="button"  data-offset="194,15">
                              <p class="card-text pb-1"><small class="text-muted"><span style="word-spacing: 15px;">Телефон: </span>
                                  <span class="badge"><i class="fas fa-phone-alt" @if($vitrina->phone_for_zvonok != null) style="font-size: 13px; color: #28a745;" @else style="font-size: 13px; color: #dc3545;" @endif></i></span>
                              </small></p>
                            </a>
                            <!-- dropdown-menu -->
                            <div class="dropdown-menu dropdown-menu-lg dropdown-menu-right pt-0 pb-0">
                              ...
                              <form class="" method="POST" action="{{ route('moderator_vitrina_update_phone_for_zvonok', $vitrina['id']) }}">
                              @csrf
                              @method('PUT')
                                <div class="row align-items-end hover1 ml-2 mr-2">
                                  ...
                                  <div class="col pl-0 pr-0">
                                    <input name="phone_for_zvonok" value="{{ $vitrina->phone_for_zvonok }}" type="text" class="form-control form-control-border  pr-1 pl-1 pt-2 pb-1 color1" data-inputmask='"mask": "(999) 999-999"' data-mask placeholder="(700) 999-999" required="true">
                                  </div>
                                </div>
                          ...
                          <div class="col border text-center shadow-sm two dropup">
                            <a class="" data-toggle="dropdown" id="dropdownMenuOffset10" type="button"  data-offset="115,15">
                              <p class="card-text pb-1"><small class="text-muted"><span style="word-spacing: 15px;">WhatsApp: </span>
                                  <span class="badge"><i class="fab fa-whatsapp" @if($vitrina->phone_for_whatsapp != null) style="font-size: 13px; color: #28a745;" @else style="font-size: 13px; color: #dc3545;" @endif></i></span>
                              </small></p>
                            </a> 
                            <!-- dropdown-menu -->
                            <div class="dropdown-menu dropdown-menu-lg dropdown-menu-right pt-0 pb-0">
                              ...
                              <div class="dropdown-divider mb-0 mt-0"></div>
                              <form class="" method="POST" action="{{ route('moderator_vitrina_update_phone_for_whatsapp', $vitrina['id']) }}">
                              @csrf
                              @method('PUT')
                                <div class="row align-items-end hover1 ml-2 mr-2">
                                  <div class="col-auto pr-0 pl-0">
                          ...
                          <div class="col border text-center shadow-sm two dropup">
                            ...
                            <p class="card-text pb-1"><small class="text-muted"><span style="word-spacing: 10px;">Баасы: <b>{{$vitrina->price / 100}}</b></span> сом</small></p>
                            </a>
                            <!-- dropdown-menu -->
                            <div class="dropdown-menu dropdown-menu-lg dropdown-menu-right pt-0 pb-0">
                              <p class="h3 dropdown-item-title text-center pt-2 pb-2 pr-2 pl-2">Баасы</p>
                              <div class="dropdown-divider mb-0 mt-0"></div>
                              <form class="" method="POST" action="{{ route('moderator_vitrina_update_price', $vitrina['id']) }}">
                              @csrf
                              @method('PUT')
                                <div class="input-group pt-2 pb-2 pr-2 pl-2">
                                  <input type="number" pattern="\d+" name="price" min="0" max="99999" maxlength="5" class="form-control" placeholder="Например: 30 " value="{{$vitrina->price / 100}}">
                          ...
                          <div class="col border text-center shadow-sm two dropup" style="border-top-right-radius: 3px; border-bottom-right-radius: 3px;">
                              ...
                              @if (intval($vitrina->status_moderator) == 0)
                            ...
                            <div class="dropdown-menu dropdown-menu-right pt-0 pb-0">
                              ...
                              <div class="dropdown-divider mb-0 mt-0"></div>
                              <form method="POST" action="{{ route('moderator_vitrina_update_status1', $vitrina['id']) }}">
                              @csrf
                              @method('PUT')
                                  ...
                                  <div class="media">
                                    <div class="media-body">
                                      <h3 class="dropdown-item-title pt-2 pb-2">
                                        Активный
                                        <span class="float-right text-sm">@if (intval($vitrina->status_moderator) == 1)<i class="far fa-check-square" style="font-size: 20px; color: #28a745;"></i>@else<i class="far fa-square" style="font-size: 20px; color: #28a745;"></i>@endif</span>
                                      </h3>
                              ...
                              <form method="POST" action="{{ route('moderator_vitrina_update_status0', $vitrina['id']) }}">
                              @csrf
                              @method('PUT')
                                ...
                                <button type="submit"  class="dropdown-item">
                                  ...
                                  <div class="media">
                                    <div class="media-body">
                                      <h3 class="dropdown-item-title pt-2 pb-2">
                                        Скрытый
                                        <span class="float-right text-sm">@if (intval($vitrina->status_moderator) == 0)<i class="far fa-check-square" style="font-size: 20px; color: #28a745;"></i>@else<i class="far fa-square" style="font-size: 20px; color: #28a745;"></i>@endif</span>
                                      </h3>
                                    </div>
                                  </div>
                                  <!-- Message End -->
                    ...
                    <div class="col-md-1 text-right tools2" style="line-height: 2.2rem; word-spacing: 30px;">
                          <!--<a class="btn btn-tool w-1001" href="{{ route('moderator_kurs_urok_index', $vitrina['id']) }}">
                            <span style="font-size: 1.2em;" data-toggle="tooltip" data-placement="left" title="Уроки даннога курса">
                              <i class="fas fa-list-ul"></i>
                            </span>
                          </a>

                          <a class="btn btn-tool" role="button" href="">
                          ...
                          <a class="btn btn-tool" href="{{ route('moderator_vitrina_edit', ['for_action' => $for_action, 'id' => $vitrina['id']]) }}">
                          ...
                          <a class="btn btn-tool w-1001 for_modal_delete" type="button" data-toggle="modal" data-target="#staticBackdrop" data-title="{{ $vitrina->title }}" data-kol="{{$vitrina->kupit_count}}" data-price="{{ $vitrina->price / 100 }}" data-id2="{{route('moderator_vitrina_destroy', $vitrina['id'])}}">
