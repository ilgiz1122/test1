...
<script src="//cdn.jsdelivr.net/npm/sweetalert2@11"></script>
  ...
  <section class="content"  id="accordion">
        ...
        <div class="col-md-7">
          <section class="content-header pl-0 pr-0">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active">{{$podcategory->title}} <span data-bs-toggle="tooltip" data-bs-placement="right" title="Сабактардын саны: {{$kol}}">({{$kol}})</span></li>
            </ol>
          </section>
        </div>
        <div class="col-md-5">
          <div class="row align-items-center">
            ...
            <div class="col-auto for_num1 for_num_id1">
              ...
              <button class="btn btn-sm btn-default float-right text-nowrap send_form_1" type="button">Сохранить изменения</button>
            </div>
            <div class="col-auto knopka_numerasi">
              <p style="display: none;"></p>
              <a class="btn btn-sm btn-default float-right text-nowrap for_button_izmenit" type="button"  onclick="changeClass()">Изменить порядок</a>
            </div>
            <div class="col-auto" >
      ...
      @endif
      <form method="POST" name="mmform" action="{{ route('moderator_kurs_uroki_num_update', ['podcat_id' => $podcategory->id]) }}">
            @csrf
            @method('PUT')
      <div class="row align-items-top">
        <div class="for_num1" id="button_for_num">
          
        </div>
        
        <div class="col">
          
            @for ($i = 1; $i <= $podcategory->col_modulei; $i++)
              ...
              <div class="{{$i}}">
                <section class="connectedSortable">
                  @foreach ($uroky->where('modul_number', $i) as $urok)
                    ...
                    <div class="row no-gutters align-items-center">
                      ...
                      <div class="col">
                        <div class="card-body pr-2 pl-2 pt-3 pb-1">
                          ...
                          <input type="number" name="modul_number[]" value="{{$urok->modul_number}}" hidden="">
                          ...
                          <p class="card-text mb-0"><small class="text-muted">Количество видеофайлов: {{$urok->youtube_ssylke_count + $urok->video_name_count}} @if($urok->ssylka != null) / <a class="btn btn-tool" href="{{ route('moderator_kurs_urok_material_download', $urok->ssylka) }}">
                                  ...
                                  <span class="float-right  tools2 for_num_id1 remove_panel">
                                    <a class="d-block w-100 btn btn-tool collapsed" data-toggle="collapse" href="#collapse{{$urok->id}}" aria-expanded="false">
                              ...
                              <div class="row mr-0 ml-0 pb-1 align-items-center remove_panel"> 
                                ...
                                <div class="col-auto tools2 for_num_id1 dropup">
                                  <a class="btn btn-tool" role="button" data-toggle="dropdown" id="dropdownMenuOffset{{$urok->id}}" type="button"  data-offset="120,0">
                                    <span style="font-size: 1.2em;" title="Задание">
                                ...
                                <div class="col-auto tools2 for_num_id1">
                                      ...
                                      ?>
                                      <div class="col-auto tools2 for_num_id1 dropup">
                                        <a class="btn btn-tool" role="button" data-toggle="dropdown" id="dropdownMenuOffset22{{$urok->id}}" type="button"  data-offset="120,0">
                        ...
                        <div id="collapse{{$urok->id}}" class="collapse tools2 for_num_id1 remove_panel" style="">
                                    ...
                                    <div class="col-auto">
                                      <div class="form-group mb-0">
                                        <div class="custom-control custom-switch block23">
                                          ...
                                          @endif
                                          <input data-id="{{$urok->id}}" type="checkbox" name="checkbox_name_{{$urok->id}}" class="custom-control-input" id="customSwitch0{{$urok->id}}" style="cursor: pointer;" @if($urok->youtube_control_status == 0) checked="" disabled=""  @endif>
                                          <label class="custom-control-label"  style="cursor: pointer;" for="customSwitch0{{$urok->id}}" data-id="{{$urok->id}}"></label>
                                        </div>
                                      </div>
                                    </div>
                                  </div>
                                    ...
                                    <div class="col-auto">
                                      <div class="form-group mb-0">
                                        <div class="custom-control custom-switch block23">
                                          ...
                                          @endif
                                          <input data-id="{{$urok->id}}" type="checkbox" name="checkbox_name_{{$urok->id}}" class="custom-control-input" id="customSwitch1{{$urok->id}}" style="cursor: pointer;" @if($urok->youtube_control_status == 1) checked="" disabled="" @endif>
                                          <label class="custom-control-label"  style="cursor: pointer;" for="customSwitch1{{$urok->id}}" data-id="{{$urok->id}}"></label>
                                        </div>
                                      </div>
                                    </div>
                                  </div>
                                    ...
                                    <div class="col-auto">
                                      <div class="form-group mb-0">
                                        <div class="custom-control custom-switch block23">
                                          ...
                                          @endif
                                          <input data-id="{{$urok->id}}" type="checkbox" name="checkbox_name_{{$urok->id}}" class="custom-control-input" id="customSwitch2{{$urok->id}}" style="cursor: pointer;" @if($urok->youtube_control_status == 2) checked="" disabled="" @endif>
                                          <label class="custom-control-label"  style="cursor: pointer;" for="customSwitch2{{$urok->id}}" data-id="{{$urok->id}}"></label>
                                        </div>
                                      </div>
                                    </div>
                                  </div>
...
<script>
  ...
  $(".fa-grip-vertical").mouseout(function() {
      const class_name = this.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.className;
      $("." + class_name).find('input[name^="modul_number[]"]').val(class_name);
  });

  $(".ssd12").mousemove(function() {
      const class_name = this.parentNode.parentNode.className;
      $("." + class_name).find('input[name^="modul_number[]"]').val(class_name);
  });
...
$('input[name="checkbox_name_{{$urok->id}}"]').on('change', function() {
    $('input[name="checkbox_name_{{$urok->id}}"]').not(this).prop('checked', false);
});
@endforeach
